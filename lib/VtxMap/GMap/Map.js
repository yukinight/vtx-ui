...
var ArcgisMap = function (_React$Component) {
    ...
    function ArcgisMap(props) {
        ...
        _this.wkid = props.wkid || 4326; //wkid 坐标系对应,全局使用.
    ...
    _createClass(ArcgisMap, [{
        ...
        value: function loadMapJs() {
            this.loadMapComplete = new Promise(function (resolve, reject) {
                if (window.GMap) {
                    ...
                    //加载 js
                    $.getScript('http://222.92.212.126:25048/gis/html/js/arcgis_js_api/library/3.9/3.9/init.js', function () {
                        var Heatmap = new Promise(function (resolve, reject) {
                            $.getScript('./resources/js/mapPlugin/gisheatmap.js', function () {
                                resolve();
                            });
                        });
                        var PointCollection = new Promise(function (resolve, reject) {
                            $.getScript('./resources/js/mapPlugin/GPointCollection.js', function () {
                                resolve();
                            });
                        });
            ...
            var event = function event() {
                ...
                var changeLabel = function changeLabel() {
                    for (var i in t.Label) {
                        if (t.Label[i]) {
                            var tl = t.state.gis.toScreen(t.Label[i].position);
                            //渲染優化
                ...
                var rangingToolFun = function rangingToolFun(e, status) {
                    if (t.rangingTool.isRanging) {
                        ...
                        if (t.rangingTool.points.length > 0) {
                            t.rangingTool.distance += (0, _MapToolFunction.getDistance)(lnlt, [t.rangingTool.points[t.rangingTool.points.length - 1].longitude, t.rangingTool.points[t.rangingTool.points.length - 1].latitude], t.state.gis);
                        } else {
                ...
                t.state.gis.on('mouse-move', function (e) {
                    ...
                    if (t.rangingTool.isRanging) {
                        ...
                        if (t.rangingTool.points.length > 0) {
                            var distance = (0, _MapToolFunction.getDistance)([e.mapPoint.x, e.mapPoint.y], [t.rangingTool.points[t.rangingTool.points.length - 1].longitude, t.rangingTool.points[t.rangingTool.points.length - 1].latitude], t.state.gis);
                            // 实时计算距离
                            distance += t.rangingTool.distance;
        ...
        value: function addMapLayers() {
            ...
            var url = [].concat(_toConsumableArray(mapServer.url || [])),
                services = [];
            //从新加载图层
            switch (mapServer.type) {
                case 'gis':
                    for (var i = 0; i < url.length; i++) {
                        var basemap = new esri.layers.ArcGISTiledMapServiceLayer(url[i]);
                ...
                case 'wmts':
                    ...
                    var tileExtent = new esri.geometry.Extent(mapServer.fullExtent.xmin, mapServer.fullExtent.ymin, mapServer.fullExtent.xmax, mapServer.fullExtent.ymax, new esri.SpatialReference({
                        wkid: mapServer.wkid
                    }));
                    var inTileExtent = new esri.geometry.Extent(mapServer.initialExtent.xmin, mapServer.initialExtent.ymin, mapServer.initialExtent.xmax, mapServer.initialExtent.ymax, new esri.SpatialReference({
                        wkid: mapServer.wkid
                    }));
                ...
                default:
                    ...
                    var dtileInfo = new esri.layers.TileInfo({
                        ...
                        "compressionQuality": 0,
                        "spatialReference": new esri.SpatialReference({
                            "wkid": _MapToolFunction.defaultWmtsMapLayers.wkid || 4326
                        }),
                        "rows": 256,
                        "cols": 256,
                    ...
                    var dtileExtent = new esri.geometry.Extent(_MapToolFunction.defaultWmtsMapLayers.fullExtent.xmin, _MapToolFunction.defaultWmtsMapLayers.fullExtent.ymin, _MapToolFunction.defaultWmtsMapLayers.fullExtent.xmax, _MapToolFunction.defaultWmtsMapLayers.fullExtent.ymax, new esri.SpatialReference({
                        wkid: _MapToolFunction.defaultWmtsMapLayers.wkid
                    }));
                    var dinTileExtent = new esri.geometry.Extent(_MapToolFunction.defaultWmtsMapLayers.initialExtent.xmin, _MapToolFunction.defaultWmtsMapLayers.initialExtent.ymin, _MapToolFunction.defaultWmtsMapLayers.initialExtent.xmax, _MapToolFunction.defaultWmtsMapLayers.initialExtent.ymax, new esri.SpatialReference({
                        wkid: _MapToolFunction.defaultWmtsMapLayers.wkid
                    }));
                    ...
                    for (var _i2 = 0; _i2 < _MapToolFunction.defaultWmtsMapLayers.url.length; _i2++) {
                        var _wmtsLayer = new esri.layers.WMTSLayer(_MapToolFunction.defaultWmtsMapLayers.url[_i2], doptions);
                        services.push(_wmtsLayer);
                    }
                    break;
            }
        ...
        value: function addPoint(mapPoints, type) {
            ...
            mapPoints.map(function (item, index) {
                ...
                if (item.markerContent) {
                    //html点位
                    //添加透明点(便于事件处理)
                    var markerSymbol = new esri.symbol.PictureMarkerSymbol('./resources/images/touming.png', cg.width, cg.height);
                    //设置偏移点的位置到左上角(原来在中心)
                    ...
                    //图片点位
                    var _markerSymbol = new esri.symbol.PictureMarkerSymbol(item.url || './resources/images/defaultMarker.png', cg.width, cg.height);
                    //设置偏移点的位置到左上角(原来在中心)
        ...
        value: function updateLine(mapLines) {
            ...
            mapLines.map(function (item, index) {
                ...
                gc._extent._parts[0].extent = _extends({}, gc._extent._parts[0].extent, _extent);
                //处理颜色和透明度  使用rgba处理透明度
                var color = new esri.Color(cg.color);
                color.a = cg.pellucidity;
        ...
        value: function addCircle(mapCircles, type) {
            ...
            mapCircles.map(function (item, index) {
                ...
                var center = new esri.geometry.Point(item.longitude, item.latitude);
                //创建圆 位置对象
                var position = new esri.geometry.Circle({ center: center, radius: item.radius });
                //线类型
                var lineStyle = esri.symbol.SimpleLineSymbol.STYLE_SOLID;
                switch (cg.lineType) {
                    case 'solid':
        ...
        */
        ...
        value: function setVisiblePoints(arg, type) {
            ...
            var ext = new esri.geometry.Extent(_extends({}, _extent, { spatialReference: { wkid: t.wkid } }));
        ...
        value: function draw(obj) {
            ...
            if (!t.drawToolbar) {
                ...
                t.drawToolbar.on('draw-complete', function (e) {
                    ...
                    if (t.drawParam.geometryType == 'circle') {
                        var radius = (0, _MapToolFunction.getDistance)([e.geometry._extent.xmin, e.geometry._extent.ymin], [e.geometry._extent.xmax, e.geometry._extent.ymax], t.state.gis) / 2;
            ...
            if (obj.geometryType !== 'point') {
                ...
                //点的样式
                var markerSymbol = new esri.symbol.PictureMarkerSymbol(obj.parameter.url || './resources/images/defaultMarker.png', obj.parameter.width || 30, obj.parameter.height || 30);
                //设置点偏移
                markerSymbol.setOffset((obj.parameter.markerContentX || -15) + (obj.parameter.width || 30) / 2, -((obj.parameter.markerContentY || -15) + (obj.parameter.height || 30) / 2));
            ...
            if (obj.geometryType == 'polygon' || obj.geometryType == 'circle' || obj.geometryType == 'rectangle') {
                ...
                //处理填充颜色和透明度  使用rgba处理透明度
                var dColor = new esri.Color(obj.parameter.color);
        ...
        value: function setAreaRestriction(areaRestriction) {
            ...
            t.areaRestriction = new esri.geometry.Extent(_extends({}, _et, { spatialReference: { wkid: t.wkid }
            }));
        }
        //关闭区域限制
        ...
        value: function calculatePointsDistance(fp, ep) {
            return (0, _MapToolFunction.getDistance)(fp, ep, this.state.gis);
        }
        //计算多个点的距离(常用于线计算)
        ...
        value: function dealLabelGraphics(id, label) {
            ...
            if (label) {
                //经纬度转top和left
                var tl = t.state.gis.toScreen(label.position),
                    lbl = _extends({}, label);
                //渲染優化
                if (!(tl.y < -50 || tl.x < -50 || tl.y > t.state.gis.height || tl.x > t.state.gis.width)) {
                    //设置label的位置(通过定位来实现)
        ...
        key: 'moveTo',
        value: function moveTo(id, lnglat, delay, autoRotation) {
            delay = delay || 3;
            var t = this,
                timer = 10;
        ...
        value: function render() {
            var t = this;
            var _map = this.props;
            return _react2.default.createElement('div', { id: _map.mapId, className: 'map' });
        }
            ...
            var initData = function initData() {
                ...
                */
                if (mapLines instanceof Array && !t.deepEqual(mapLines, t.props.mapLines)) {
                    ...
                    t.addLine(_addedData2);
                    //更新
                    t.updateLine(_updatedData2);
                }
                //画其他特例线专用
                ...
                */
                if (mapPolygons instanceof Array && !t.deepEqual(mapPolygons, t.props.mapPolygons)) {
                    ...
                    t.addPolygon(_addedData4);
                    t.updatePolygon(_updatedData4);
                }
                /*
                    圆数据处理
                    先全删除,再新增
                */
                if (mapCircles instanceof Array && !t.deepEqual(mapCircles, t.props.mapCircles)) {
                    ...
                    t.addCircle(_addedData5);
                    t.updateCircle(_updatedData5);
                }
                //绘制边界线
