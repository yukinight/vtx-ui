    ...
    _createClass(TMap, [{
        ...
        value: function loadMapJs() {
            this.loadMapComplete = new Promise(function (resolve, reject) {
                if (window.T) {
                    ...
                    $.getScript('http://api.tianditu.com/getscript?v=4.0', function () {
                        resolve(window.T);
                    });
                }
            });
        ...
        value: function createMap() {
            ...
            window.VtxMap[mapId] = t.state.gis = new T.Map(mapId, {
                ...
                center: mapCenter ? new T.LngLat(mapCenter[0], mapCenter[1]) : new T.LngLat(116.40769, 39.89945),
                minZoom: minZoom || 1,
                maxZoom: maxZoom || 18
            });
            t.state.gis.addEventListener('load', function (e) {
                console.log('load', e);
            });
        }
        //清空地图所有图元
        ...
        */
            ...
            switch (obj.geometryType) {
                case 'point':
                    param.icon = new T.Icon({
                        ...
                        iconAnchor: new T.Point(obj.parameter.width ? obj.parameter.width / 2 : 16.5, obj.parameter.height ? obj.parameter.height : 33)
                    });
                    param.follow = false;
        ...
        value: function calculatePointsDistance(f, s) {
            var lnglat = new T.LngLat(f[0], f[1]);
        ...
        value: function render() {
            var t = this;
            return _react2.default.createElement('div', { id: t.props.mapId, style: { width: '100%', height: '100%' } });
        }
    }, {
        ...
        value: function componentWillReceiveProps(nextProps, prevProps) {
            ...
            */
            if (mapLines instanceof Array && !t.deepEqual(mapLines, t.props.mapLines)) {
                ...
                t.updateLine(_updatedData);
                t.addLine(_addedData);
            }
            //画其他特例线专用
            if (customizedBoundary instanceof Array && !t.deepEqual(customizedBoundary, t.props.customizedBoundary)) {
                var _t$dataMatch3 = t.dataMatch(t.props.customizedBoundary, customizedBoundary, 'id'),
            ...
            */
            if (mapPolygons instanceof Array && !t.deepEqual(mapPolygons, t.props.mapPolygons)) {
                ...
                t.updatePolygon(_updatedData3);
                t.addPolygon(_addedData3);
            }
            /*
                圆数据处理
                先全删除,再新增
            */
            if (mapCircles instanceof Array && !t.deepEqual(mapCircles, t.props.mapCircles)) {
                ...
                try {
                    for (var _iterator5 = _deletedDataIDs4[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
                ...
                t.updateCircle(_updatedData4);
                t.addCircle(_addedData4);
            }
            // //绘制边界线
