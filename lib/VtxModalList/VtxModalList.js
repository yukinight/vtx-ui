    ...
    _createClass(VtxModalList, [{
        ...
        value: function renderChildren() {
            ...
            if (!!chil) {
                        ...
                        if (!!item) {
                            ...
                            if (typeof item === 'function') {
                                ...
                                if (Array.isArray(_vchil)) {
                                    return clone(_vchil, '' + key + (modalListKey || index));
                                } else {
                                    return clone([_vchil], '' + key + (modalListKey || index));
                                }
                            }
                ...
                if ((typeof chil === 'undefined' ? 'undefined' : _typeof(chil)) === 'object' && !(chil instanceof Array)) {
                    if (chil.props) {
                        cC = clone([chil], 'root');
                    } else {
                        var vchil = [];
                        for (var i in chil) {
                            if (_typeof(chil[i]) === 'object') {
                                vchil.push(chil[i]);
                            }
                        }
                        cC = clone(vchil, 'root');
                ...
                if (chil instanceof Array) {
                    ...
                    //清空验证缓存
                    for (var _i in orl) {
                        if (!newRepeteKeyList.includes(_i)) {
                            t.repeteList[_i] = {};
                        }
                    }
                }
                        ...
                        if ((r.value || r.value == 0) && !(typeof r.value == 'string' && !r.value.trim())) {
                            ...
                            if (!!reg.exp) {
                                if (reg.exp instanceof RegExp) {
                                    ...
                                    for (var _i2 = 0; _i2 < reg.exp.length; _i2++) {
                                        if (reg.exp[_i2] instanceof RegExp) {
                                            reg.exp[_i2].lastIndex = 0;
                                            required = reg.exp[_i2].test(r.value);
                                        } else if (reg.exp[_i2] instanceof Function) {
                                            required = reg.exp[_i2](r.value);
                                        }
                                        if (!required) {
                                            break;
                                        }
                    ...
                    return Promise.all(plist).then(function (values) {
                        if (ind >= t.allRecord) {
                            ...
                            for (var _i3 = 0; _i3 < values.length; _i3++) {
                                t.repeteList[ii[_i3]] = _extends({}, t.repeteList[ii[_i3]], {
                                    isRepete: values[_i3].data.data,
                                    errorMsg: values[_i3].data.msg || ''
                                });
                                if (!values[_i3].data.data) {
                                    isRequest = false;
                                }
                            }
