...
var VtxModalList = function (_React$Component) {
    ...
    _createClass(VtxModalList, [{
        ...
        value: function renderChildren() {
            ...
            //清空缓存,避免缓存数据
            if (!!chil) {
                var classify = function classify(c) {
                    if ((typeof c === 'undefined' ? 'undefined' : _typeof(c)) === 'object' && !(c instanceof Array)) {
                        if (c.props) {
                            return _clone([c], 'root');
                        } else {
                            var vchil = [];
                            for (var i in c) {
                            ...
                            return _clone(vchil, 'root');
                        }
                    }
                    if (typeof c === 'function') {
                        var _vchil = c();
                        if (Array.isArray(_vchil)) {
                            return _clone(_vchil, 'root');
                        } else {
                            return _clone([_vchil], 'root');
                        }
                    }
                ...
                //复制子节点处理数据
                var _clone = function _clone(ary, key) {
                        ...
                        if (!!item) {
                            if (item.type && item.type.toString().indexOf('fragment') > -1) {
                                ...
                                if (item instanceof Array) {
                                    return _clone(item, '' + key + (modalListKey || index));
                                }
                                if ((typeof item === 'undefined' ? 'undefined' : _typeof(item)) === 'object') {
                                    if (!item.props) {
                                        ...
                                        return _clone(vchil, '' + key + (modalListKey || index));
                                    }
                                }
                                if (typeof item === 'function') {
                                    var _vchil2 = item();
                                    if (Array.isArray(_vchil2)) {
                                        return _clone(_vchil2, '' + key + (modalListKey || index));
                                    } else {
                                        return _clone([_vchil2], '' + key + (modalListKey || index));
                                    }
                                }
        ...
        value: function cloneComponent(elem, index) {
            ...
            //     // }
            // }}:{}
            typeof elem.props.onChange == 'function' ? { onChange: elem.props.onChange } : {}, elem.props.prefixCls && elem.props.prefixCls == "ant-input" || (mld.layout || {}).comType == 'input' ? { onFocus: function onFocus(e) {
                ...
                } } : {}, elem.props.prefixCls && elem.props.prefixCls == "ant-input" || (mld.layout || {}).comType == 'input' ? { placeholder: elem.props.placeholder || '请录入' + mld.layout.name } : {}, elem.props.prefixCls && elem.props.prefixCls == "ant-select" || (mld.layout || {}).comType == 'select' || elem.type.name === 'SelectZTree' ? { placeholder: elem.props.placeholder || '请选择' + mld.layout.name } : {}));
