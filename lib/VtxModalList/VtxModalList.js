                ...
                for (var i in t.repeteList) {
                    ...
                    if (r.type == 'default') {
                        ...
                        if (r.value && !(typeof r.value == 'string' && !r.value.trim())) {
                            ...
                            if (!!_reg.exp) {
                                if (_reg.exp instanceof RegExp) {
                                    required = _reg.exp.test(r.value);
                                } else if (_reg.exp instanceof Function) {
                                    required = _reg.exp(r.value);
                                } else if (_reg.exp instanceof Array) {
                                    for (var _i = 0; _i < _reg.exp.length; _i++) {
                                        if (_reg.exp[_i] instanceof RegExp) {
                                            required = _reg.exp[_i].test(r.value);
                                        } else if (_reg.exp[_i] instanceof Function) {
                                            required = _reg.exp[_i](r.value);
                                        }
                                        if (!required) {
                                            break;
                                        }
